// P4: a REAL project source converted to a module, changing nothing but the
// scaffolding. This is the mechanical recipe the other 437 business files will
// follow, so it has to be validated on real code rather than a toy:
//
//   #pragma once                ->  export module utils.throttle;
//   #include "vendor/std.hpp"   ->  import std;
//   namespace utils::throttle { ->  export namespace utils::throttle {
//
// Everything below the scaffolding is byte-identical to
// src/utils/throttle/throttle.hpp. It exercises variadic templates, an
// explicit class specialisation (ThrottleState<void>), an explicit function
// template specialisation (create<void>), std::mutex/lock_guard, chrono and
// std::apply — a dense enough slice of the codebase to be a real answer.
