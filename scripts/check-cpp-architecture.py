...
One sibling check owns the rest: `check-module-graph.py` — the graph is a DAG,
and every entity a unit names is exported by a module it can see.
"""
...
def validate_file(path: Path, errors: list[str]) -> None:
    ...
    if not is_module_unit and path.suffix == ".cpp":
        ...
        #
        # The other direction merges fine: import-first fails, include-first
        # passes, and nothing else was changed between the two.
        first_import = None
